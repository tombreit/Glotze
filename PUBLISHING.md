...
1. You write a **flatpak-builder manifest** (already done — `data/io.github.tombreit.Glotze.yml`).
...
- The manifest (`data/io.github.tombreit.Glotze.yml`) no longer asks for `--share=network`, sets `CARGO_NET_OFFLINE=true`, runs `cargo build --release --offline --locked`, and pulls in `../cargo-sources.json` as a second source.
...
flatpak-builder --show-manifest data/io.github.tombreit.Glotze.yml > /dev/null
```
...
### Step 6 — Optional: migrate to meson as the outer build

Every reference app on Flathub uses meson, not raw cargo, as the outer build system. Reasons:

- Free `appstreamcli validate` and `desktop-file-validate` as `meson test` targets.
- `gnome.post_install()` updates the GTK icon cache, the desktop database, and any GSettings schemas automatically — without it, your icon may not appear in the app grid until the user logs out.
- A clear path to add gettext translations later.

This is a real refactor but the diff is small for an app this size. Look at Loupe's top-level `meson.build` and `data/meson.build` — copy the shape. Inside meson, you still call `cargo build --release` via a `custom_target`.

Not blocking for the first Flathub PR. You can submit with `buildsystem: simple` and migrate later.
...
4. Add the following files at the repo root of your branch:
   - `io.github.tombreit.Glotze.yml` (the manifest, copied from your `data/`)
   - `cargo-sources.json`
