...
# .desktop and .metainfo.xml are generated by meson's i18n.merge_file from
# these .in templates. Validating the templates is sufficient locally — the
# merge only appends xml:lang="…" siblings / Name[de]= keys, which doesn't
# affect schema validity. The CI `flatpak-builder-lint repo` step (and the
# in-sandbox meson tests via `run-tests: true`) validate the merged output.
METAINFO="data/io.github.tombreit.Glotze.metainfo.xml.in"
DESKTOP="data/io.github.tombreit.Glotze.desktop.in"
...
if command -v desktop-file-validate >/dev/null 2>&1; then
    ...
    # desktop-file-validate rejects the .in suffix; copy to a tmp .desktop
    # name so it'll inspect the content. The meson merge only appends
    # Name[de]= etc., which doesn't change validity.
    tmp_desktop="$(mktemp --suffix=.desktop)"
    trap 'rm -f "$tmp_desktop"' EXIT
    cp "$DESKTOP" "$tmp_desktop"
